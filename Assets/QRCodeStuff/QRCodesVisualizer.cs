    ...
    public class QRCodesVisualizer : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            foreach (var qrcode in qrCodesObjectsList)
            {
                ...
                statusText.text = "qrcode.Data " + qrcode.Value.gameObject.transform.localPosition.ToString();
                TestCube.transform.position = qrcode.Value.gameObject.transform.localPosition;
            }
        }
        ...
        public void CustomedStopScan(string qrcode_text)
        {
            foreach (var qrcode in qrCodesObjectsList)
            {
                Debug.Log("qrcode.Data " + qrcode.Key.ToString());
                if (qrcode.Key.ToString() == qrcode_text)
                {
                    model.transform.localPosition = new Vector3(qrcode.Value.gameObject.transform.localPosition.x,
                        qrcode.Value.gameObject.transform.localPosition.y + (float)1.5,
                        qrcode.Value.gameObject.transform.localPosition.z);
                    model.transform.localRotation = qrcode.Value.gameObject.transform.localRotation;
                    model.transform.Rotate(new Vector3(90, 0, 0));

                    //Quaternion tmp_rotation = Quaternion.AngleAxis(90f, Vector3.right);
                    //model.transform.localEulerAngles = tmp_rotation * qrcode.Value.gameObject.transform.localEulerAngles;
                    //model.transform.localEulerAngles = new Vector3(qrcode.Value.gameObject.transform.localEulerAngles.x,
                    //    qrcode.Value.gameObject.transform.localEulerAngles.x,
                    //   qrcode.Value.gameObject.transform.localEulerAngles.z);

                    is_scan_success = true;

                    statusText.text = "qrcode pos: " + qrcode.Value.gameObject.transform.localPosition + "qrcode rot: " +
        ...
        public bool IsQRScanSucessful()
        {
            return is_scan_success;
        }
